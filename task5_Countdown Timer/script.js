const hour = document.getElementById('hour');
const minute = document.getElementById('minute');
const second = document.getElementById('second');

const start = document.getElementById('start');
const stop = document.getElementById('stop');
const reset = document.getElementById('reset');
const restart = document.getElementById('restart');
const disp = document.getElementById('disp');

var interval = null;
var total = 0;
var hr_earlier = 0;
var min_earlier = 0;
var sec_earlier = 0;


totalValue = ()=>{
    total = Number(hour.value)*3600 + Number(minute.value)*60 + Number(second.value);
}

Timer = ()=>{
    totalValue();
    total--;

    if(total >= 0){
        var hr = Math.floor(total/3600);
        var mt = Math.floor((total/60) - (hr*60));
        var sc = total - ((hr*3600) + (mt*60));

        hour.value = hr;
        minute.value = mt;
        second.value = sc;
    }
    else{
        disp.innerText = "Time Over !!"
    }
}


start.addEventListener('click', ()=>{
    hr_earlier = hour.value;
    min_earlier = minute.value;
    sec_earlier = second.value;
    clearInterval(interval);
    interval = setInterval(Timer, 1000);

    disp.innerText = "Timer Started";
});

reset.addEventListener('click', ()=>{
    clearInterval(interval);

    hour.value = 0;
    minute.value = 0;
    second.value = 0;

    disp.innerText = "Timer";

});

stop.addEventListener('click', ()=>{
    clearInterval(interval); 
    disp.innerText = "Timer Stopped";
} )

restart.addEventListener('click', () => {
    hour.value = hr_earlier;
    minute.value = min_earlier;
    second.value = sec_earlier;
    clearInterval(interval);
    totalValue(); // Ensure the total is updated before restarting
    interval = setInterval(Timer, 1000);
    disp.innerText = "Timer Restarted";
  });
